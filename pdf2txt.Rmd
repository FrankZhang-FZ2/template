...
pdf_path <- file.path("D:", "Work", "EAS project", "Text scraping", "data","PDFs","Strangers_drowning.pdf")
# extract text
...
txt_output_meaningful <- txt_output %>%
  substr(start=1557, stop=590710) %>%
  as.data.frame() %>%
  flextable::flextable() %>%
  flextable::set_table_properties(width = .95, layout = "autofit") %>%
  ...
  flextable::fontsize(size = 12, part = "header") %>%
  flextable::align_text_col(align = "center") %>%
  flextable::set_caption(caption = "Chapters in the book")  %>%
  flextable::border_outer()

print(txt_output_meaningful)
